...
export class FiltersService {
  ...
  async create(createFilterDto: CreateFilterDto) {
    ...
    if (existing) {
      throw new ConflictException(`Filter with key ${createFilterDto.key} already exists`);
    }
  ...
  async update(id: string, updateFilterDto: UpdateFilterDto) {
    const filter = await this.prisma.filterDefinition.findUnique({ where: { id } });
    if (!filter) throw new NotFoundException(`Filter #${id} not found`);
    ...
    if (updateFilterDto.key && updateFilterDto.key !== filter.key) {
      ...
      if (existing) throw new ConflictException(`Filter key ${updateFilterDto.key} is taken`);
    }
