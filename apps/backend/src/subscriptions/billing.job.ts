  ...
  async handleSubscriptionBilling() {
    ...
    try {
      ...
      const dueSubscriptions =
        await this.subscriptionRepo.findDueSubscriptions(cycleKey);
      this.logger.log(
        `Found ${dueSubscriptions.length} subscriptions to bill.`,
      );
      ...
      for (const sub of dueSubscriptions) {
        try {
          ...
          this.logger.log(
            `Successfully generated Order ${order.id} for Subscription ${sub.id}`,
          );
          ...
          if (sub.user.email) {
            this.emailService
              ...
              .catch((err) => {
                this.logger.error(
                  `Failed to send invoice email to ${sub.user.email}: ${err.message}`,
                );
              });
          }
        } catch (error) {
