  ...
  const categories = [
    {
      ...
      icon: <Package className="w-8 h-8 text-primary-600 mb-4" />,
      link: '/track-order'
    },
    {
      title: 'Returns & Refunds',
      description: 'Start a return, track refund status, and read our policies.',
      icon: <RefreshCcw className="w-8 h-8 text-primary-600 mb-4" />,
      link: '/track-return'
    },
    {
      title: 'FAQs',
      description: 'Find quick answers to common questions about our services.',
      icon: <HelpCircle className="w-8 h-8 text-primary-600 mb-4" />,
      link: '/support/faq'
    },
    {
      title: 'Policies',
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="relative max-w-2xl mx-auto">
            ...
            <input 
              ...
              className="w-full bg-white pl-14 pr-6 py-4 rounded-full text-lg text-gray-900 focus:outline-none focus:ring-4 focus:ring-primary-500/50 shadow-lg"
            />
            
            {/* Live Search Results */}
            {searchQuery.trim() !== '' && (
              <div className="absolute top-full mt-2 w-full bg-white rounded-xl shadow-2xl border border-gray-100 overflow-hidden text-left z-50 max-h-96 overflow-y-auto">
                {filteredFaqs.length > 0 ? (
                  <ul className="divide-y divide-gray-100">
                    {filteredFaqs.map(faq => (
                      <li key={faq.id}>
                        <Link href={`/support/faq#faq-${faq.id}`} className="block px-6 py-4 hover:bg-gray-50 transition-colors">
                  ...
                  <div className="p-6 text-center text-gray-500">
                    ...
                    <br />
                    <Link href="/support/contact" className="text-primary-600 font-medium hover:underline mt-2 inline-block">Contact Support</Link>
                  </div>
                )}
              </div>
            )}
      ...
      <div className="max-w-6xl mx-auto px-4 -mt-10 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {categories.map((cat, i) => (
            <Link key={i} href={cat.link} className="bg-white rounded-2xl p-8 shadow-sm hover:shadow-md transition-shadow border border-gray-100 flex flex-col items-center text-center group">
