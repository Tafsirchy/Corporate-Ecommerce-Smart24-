    ...
    <div className="container mx-auto px-4 py-8 flex-1">
      ...
      {!isInitialized ? (
        <div className="space-y-4">
          {[1, 2, 3].map((n) => (
            <div key={n} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden h-32 animate-pulse flex">
              <div className="bg-gray-200 h-full w-32"></div>
          ...
          {items.map((item) => (
            <div key={item.productId} className="flex flex-col sm:flex-row items-center gap-4 bg-white p-4 rounded-lg shadow-sm border border-gray-100 hover:border-primary-100 transition">
              ...
              <div className="flex-1 w-full flex flex-col justify-between py-1 h-full min-w-0">
                <div>
                  <Link href={`/shop/${item.product?.slug}`} className="text-lg font-medium text-gray-900 hover:text-primary-600 line-clamp-1">
                    {item.product?.name}
                  </Link>
