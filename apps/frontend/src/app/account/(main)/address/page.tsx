  ...
  return (
    <>
      ...
      <div className="w-full">
          ...
          {addresses.length === 0 ? (
            <div className="md:hidden bg-white border border-border p-6 text-center text-base text-muted-foreground">
                ...
                <li key={address.id} className="bg-white border border-border p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <button 
                      ...
                      className="flex min-h-11 min-w-16 items-center justify-center px-3 text-base text-primary hover:underline uppercase"
                    >
                      Edit
                    </button>
                  </div>
                  <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-border pt-3">
                    {address.isDefaultShipping ? (
                      <span className="flex min-h-11 items-center text-sm font-semibold text-foreground">Default Shipping</span>
                    ) : (
                      <button onClick={() => handleMakeDefaultShipping(address.id)} className="flex min-h-11 items-center px-3 text-sm text-primary hover:underline">
                        Make default shipping
                      </button>
                    )}
                    {address.isDefaultBilling ? (
                      <span className="flex min-h-11 items-center text-sm font-semibold text-foreground">Default Billing</span>
                    ) : (
                      <button onClick={() => handleMakeDefaultBilling(address.id)} className="flex min-h-11 items-center px-3 text-sm text-primary hover:underline">
      ...
      {/* Address Modal */}
      {isModalOpen && (
        <div role="dialog" aria-modal="true" aria-label={editingId ? 'Edit address' : 'Add new address'} className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
          <div className="bg-white shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto pb-[env(safe-area-inset-bottom)]">
            <div className="p-6 border-b flex justify-between items-center">
              <h2 className="text-xl font-semibold">{editingId ? 'Edit Address' : 'Add New Address'}</h2>
              <button onClick={handleCloseModal} aria-label="Close dialog" autoFocus className="flex h-11 w-11 items-center justify-center text-muted-foreground hover:text-black">
            ...
            <form onSubmit={handleSaveAddress} className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input id="address-postcode" required type="text" value={formData.postcode} onChange={(e) => setFormData({...formData, postcode: e.target.value})} autoComplete="postal-code" className="h-12 w-full border border-input px-3 text-base focus:border-primary" />
                </div>
                <div>
                  <label htmlFor="address-phone" className="block text-base font-medium text-foreground mb-2">Phone Number</label>
                  <input id="address-phone" required type="text" value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} autoComplete="tel" className="h-12 w-full border border-input px-3 text-base focus:border-primary" />
                </div>
                <div>
